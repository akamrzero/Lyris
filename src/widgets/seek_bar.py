...
from ..utils.time_converter import ms_to_minutes
...
class SeekBar(Gtk.Box):
    def __init__(self):
        ...
        self._seekbar.set_adjustment(self._adjustment)
        self._seekbar.set_orientation(Gtk.Orientation.HORIZONTAL)
        self.append(self._seekbar)

        self.time_1_label = Gtk.Label(halign=Gtk.Align.START)
        self.time_2_label = Gtk.Label(halign=Gtk.Align.END, hexpand=True)

        bottom_wrapper = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL)
        bottom_wrapper.append(self.time_1_label)
        bottom_wrapper.append(self.time_2_label)
        self.append(bottom_wrapper)
    ...
    def duration(self, value):
        self._adjustment.set_upper(value)
        self.time_2_label.set_label(ms_to_minutes(value))
    ...
    def value(self, value):
        if not self._seeking:
            self._adjustment.set_value(value)
            self.time_1_label.set_label(ms_to_minutes(value))
    ...
    def create_click_gesture(self):
        click_gesture = None
        controllers = self._seekbar.observe_controllers()
        for i in range(controllers.get_n_items()):
            controller = controllers.get_item(i)
            if isinstance(controller, Gtk.GestureClick):
                click_gesture = controller
